    ...
    class InstagramAPIMethod(object):
            ...
            if status_code == 200:
                ...
                if self.response_type == 'list':
                    for entry in content_obj['data']:
                        ...
                        json_del_post = simplejson.dumps(entry)
                        ...
                        lista_jsons.append(json_del_post)
                    ...
                    pass
                # pancho modificacion
                return api_responses, self._build_pagination_info(content_obj), content, lista_jsons
            else:
        ...
        def execute(self):
            ...
            if self.as_generator:
                ...
                content, json_pancho, next, lista_jsons = self._do_api_request(url, method, body, headers)
            if self.paginates:
                return content, json_pancho, next, lista_jsons
            else:
                return content, json_pancho, lista_jsons
            # Pancho - Modificaciones
            # 1) Modifique el return
            # en _do_api_request agrege el "content" al return (al final)
            # se recibe en json pancho en estos 3 ultimos return
