...
def main():
	...
	config = "conf/twitter.cfg"

	cfg = ConfigParser.ConfigParser()
	try:
		cfg.readfp(file(config))
	except Exception, e:
		print "Error leyendo fichero de configuración ", config
		sys.exit(1)

	oauth_handler1  = cfg.get("user", "oauth_handler1")
	oauth_handler2  = cfg.get("user", "oauth_handler2")

	access_token1   = cfg.get("user", "access_token1")
	access_token2   = cfg.get("user", "access_token2")
	...
	NOMBRE_ARCHIVO_ENTRADA = cfg.get("files", "input")
	#except:
	...
	NOMBRE_ARCHIVO_SALIDA_USER 		= cfg.get("files", "output_usuario")
	NOMBRE_ARCHIVO_SALIDA_HASHTAG	= cfg.get("files", "output_hashtag")
				
	#except:
	#	NOMBRE_ARCHIVO_SALIDA = "./salida/postsTwitter.js"

	try:	
		SINCE_ID_VAR = str(sys.argv[1])
	except:
		SINCE_ID_VAR = None

	print "NOMBRE_ARCHIVO_ENTRADA: " + NOMBRE_ARCHIVO_ENTRADA
	print "NOMBRE_ARCHIVO_SALIDA_USER: " + NOMBRE_ARCHIVO_SALIDA_USER
	print "NOMBRE_ARCHIVO_SALIDA_HASHTAG: " + NOMBRE_ARCHIVO_SALIDA_HASHTAG
	print "SINCE_ID_VAR: " + str(SINCE_ID_VAR)





	escritor_user 		= open(NOMBRE_ARCHIVO_SALIDA_USER,'w')
	escritor_hashtag 	= open(NOMBRE_ARCHIVO_SALIDA_HASHTAG,'w')
	...
	for inusuario in usuarios:
		...
		for status in Cursor(api.user_timeline, user.id, since_id = SINCE_ID_VAR).items(CANT_TWEETS):
			...
			escritor_user.write(tweetGuardar.to_JSON_one_line())
			escritor_user.write('\n')
		...
		for intag in tagsuser:
			...
			for tweets in Cursor(api.search, q=intag, since_id=SINCE_ID_VAR).items(CANT_TWEETS):
				...
				escritor_hashtag.write(tweetGuardar.to_JSON_one_line())
				escritor_hashtag.write('\n')
	...
	escritor_user.write("!@#END\n")
	escritor_user.write(str(next_begin))
	
	escritor_hashtag.write("!@#END\n")
	escritor_hashtag.write(str(next_begin))
