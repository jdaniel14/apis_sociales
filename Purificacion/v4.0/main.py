...
def mainFacebook(escritor):
	...
	for line in lines:
		...
		if ((line[0] == '@') or (line[0] == '#')):
			...
			if (last == "@"):
				pub.post_network		= "Facebook"
			else:
				pub.post_network		= var_json["application"]["name"]
			...
			if ("message" in var_json.keys()):
				if(var_json["message"] == None):
					...
					pub.post_text			= var_json["message"]
			elif ("caption" in var_json.keys()):
				if(var_json["caption"] == None):
					pub.post_text = ""
				else:
					pub.post_text			= var_json["caption"]
			else: # existen casos de videos en el que no hay texto ni nada
				pub.post_text = ""
			
			mensaje_post = pub.post_text

			#seccion del user
			user_id				= var_json["from"]["id"]
			user_screen_name	= None #username no disponible
			user_name			= var_json["from"]["name"]
			...
			for word in mensaje_post.encode('utf-8').split():
				if word.startswith('#'):
					inicio = pub.post_text.encode('utf-8').lower().find(word.lower())
					fin = inicio + len(word)
					indices	= (inicio, fin)
					text	= word
					list_hashtags.append({"text":text, "indices":indices})
