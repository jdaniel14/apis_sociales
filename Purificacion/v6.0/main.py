...
# python entrada_twitter entrada_instagram entrada_facebook nombre_archivo_salida
# si se quiere obviar uno de los archivos, se coloca ""
...
def mainTwitter(escritor, inputArch):

	with open(inputArch,'r') as f:
		lines = f.readlines()
...
def mainInstagram(escritor, inputArch):
...
def mainFacebook(escritor,inputArch):
	with open(inputArch,'r') as f:
		lines = f.readlines()
...
def main():
	...
	try:
		ENTRADA_TWT = str(sys.argv[1])
	except:
		ENTRADA_TWT = "postsTwitter.txt"

	try:	
		ENTRADA_INS = str(sys.argv[2])
	except:
		ENTRADA_INS = "postsInstagram.txt"


	try:	
		ENTRADA_FBK = str(sys.argv[3])				
	except:
		ENTRADA_FBK = "postsFacebook.txt"

	try:	
		ARCHIVO_SALIDA = str(sys.argv[4])

		if(ARCHIVO_SALIDA == ""):
			ARCHIVO_SALIDA = "pe_publicaciones_fecha_hora.txt"
	...
	#Archivo de salida
	escritor = open(ARCHIVO_SALIDA,'w')


	if not(ENTRADA_TWT == ""):
		mainTwitter(escritor, ENTRADA_TWT)

	if not(ENTRADA_INS == ""):
		mainInstagram(escritor, ENTRADA_INS)

	if not(ENTRADA_FBK == ""):
		mainFacebook(escritor, ENTRADA_FBK)
